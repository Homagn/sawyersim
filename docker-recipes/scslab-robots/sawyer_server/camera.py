...
class Camera:
  def __init__(self, imtype, topic_desc = "color"): #can change the topic_desc to "color_side" for other camera view
    ...
    if imtype =="raw":
        ...
        rospy.Subscriber('camera/'+topic_desc+'/image_raw',Image, self.image_callback)
        #rospy.Subscriber('/io/internal_camera/'+topic_desc+'/image_raw',Image, self.image_callback)
    if imtype =="compressed":
        #rospy.Subscriber('camera/rgb/image_raw/compressed',CompressedImage, self.comp_image_callback)
        #rospy.Subscriber('camera/color/image_raw/compressed',CompressedImage, self.comp_image_callback)
        rospy.Subscriber('camera/'+topic_desc+'/image_raw/compressed',CompressedImage, self.comp_image_callback)
        #rospy.Subscriber('/io/internal_camera/'+topic_desc+'/image_raw/compressed',CompressedImage, self.comp_image_callback)
  def see(self):
